...
export function fragmentSource() {
    ...
    const fsSource = `
                      ...
                      float b)  // sun light direction
        {
            ...
            vec3  fogColor  = mix(vec3(0.5, 0.8, 0.3), // purple
                                  vec3(0.7, 1.0, 0.7), // blue
                                  pow(sunAmount, 8.0));
        ...
        vec3 colourScene(vec3 ro, vec3 rd, Scene scene) {
            ...
            if (scene.id == FLOOR) {
                ...
                pc += vec3(0.5, 1.0, 0.3) * clamp(flrTex.x, 0.0, 1.0);
                ...
                pc = vec3(0.1) * diff;
                pc += vec3(0.6, 1.0, 0.3) * step(a, 0.3) * tex(vec2(length(rp.xy), a));
                pc += vec3(1.0) * spec;
